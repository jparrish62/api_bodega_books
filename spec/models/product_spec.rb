...
RSpec.describe Product, type: :model do
  ...
  describe "filter_by_title" do
    ...
    context "when a 'TV' title pattern is sent" do
      it "returnss the 2 products matching" do
        expect(Product.filter_by_title("TV")).to have(2).tiems
      end
  ...
  describe ".above_or_equal_price" do
    before(:each) do
  ...
  describe "recent" do
    ...
    it "returns the most updated records" do
      expect(Product.recent).to match_array([@product1, @product2, @product3, @product4])
    end
  end
end
